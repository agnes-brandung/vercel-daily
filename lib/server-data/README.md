...
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `**getArticles`**     | Fetches all articles once, returns **parsed** articles plus **derived lists** (via pure helpers in `articleListDerivations.ts`) — home, catalog, search.                                                                                                              |
...
| `**getArticle`**             | Fetches a **single article** by **slug or id** from `GET /articles/{key}`; returns `**GetArticleResult`** (`{ article, error? }`) — `article` is `null` when missing (404); `error` when the request failed otherwise. Tags: `articles`, `all-articles`, `article:…`. |
